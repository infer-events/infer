...
1. **`get_project_summary`** — Full context first. Health score, key metrics, event catalog, funnel performance, active threads with annotations. This is the analytics wiki — compiled hourly, always up to date.

2. **`get_insights`** — Specific alerts second. Shows anomalies, volume drops, error spikes with thread context (day count, prior annotations, thread ID). After investigating, file your findings back.

3. **`annotate_thread`** — After investigating an alert, record what you found. Pass the `thread_id` from `get_insights` output and your finding as `content`. This compounds — future sessions see your notes immediately.

```
Session flow:
  get_project_summary  →  "health 7/10, 2 active issues, signup trending down"
  get_insights         →  "[!!] signup dropped 42%, Day 3, Thread: t-abc-123"
  (investigate)        →  run git log, check deploy history, query events
  annotate_thread      →  "Root cause: deploy abc123 broke OAuth callback"
  
Next session (or next agent):
  get_project_summary  →  thread now shows "↳ Root cause: deploy abc123..."
  get_insights         →  "Note: Root cause: deploy abc123..."
                          No re-investigation needed — context is already there.
```

### Use `get_ontology` / `update_ontology` for:
- Viewing how events are classified (activation, engagement, monetization, referral, noise)
- Classifying events into product categories and funnel stages
- Use `proposed` status when suggesting, `confirmed` when the developer agrees
...
1. **`get_project_summary`** — Full picture: health score, metrics, trends, active threads
2. **`get_insights`** — What's new since last check? Any critical alerts?
3. **Investigate** any critical/notable insights using `get_event_counts`, `git log`, etc.
4. **`annotate_thread`** — Record findings for any investigated threads
5. **Deep dive** if needed: `get_retention`, `get_user_journey` for specific questions

Report format:
```
Product Health — [date]

Health: [X/10] ([N] active issues)
Events: [N] ([+/- X%] vs prior 30d)
Users: [N] ([+/- X%])
Error rate: [X%]

Active threads:
  🔴 [thread title] (day N) — [your annotation or "investigating"]
  🟡 [thread title] — [status]

Actions taken:
  - [what you investigated and annotated]
```

### Scheduled / Recurring Checks

For scheduled agents (cron, remote MCP), use this flow:
1. `get_project_summary` — the single source of truth
2. `get_insights` — only if summary shows active issues
3. `annotate_thread` — record any automated findings

The project summary is the right tool for scheduled checks because it's pre-compiled (fast), includes everything (metrics + threads + annotations), and doesn't auto-ack insights (so human agents can still see them).
